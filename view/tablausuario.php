...
<div class="card shadow mb-4">
    <div class="card-header d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold ">Registro de Usuarios</h6>
        <button class="btn btn-outline-info" data-toggle="modal" data-target="#modalAgregarUsuario">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" style="text-align: center;" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody style="color: #545454">
                    <?php
                    $dato = new Usuario();
                    $usuario = $dato->obtenerUsuario();
                    $cont = 0;
                    while ($row = mysqli_fetch_array($usuario)) {
                        $idusuario = $row['USU_ID'];
                        $cont = $cont + 1;
                        $datos = $row["USU_ID"] . "||" . $row["USU_NOMBRE"] . "||" . $row["USU_APELLIDO"] . "||" . $row["USU_CORREO"] . "||" . $row["USU_PASSWORD"] . "||" . $row["USU_IDENTIFICACION"] . "||" . $row["USU_TELEFONO"] . "||" . $row["USU_DIRECCION"] . "||" . $row["ROL_ID"];
                    ?>
                        <tr>
                            <td><?php echo $cont; ?></td>
                            <td>
                                <?php if ($row['ROL_ID'] == 1) { ?>
                                    <img class="img-profile rounded-circle" style="width: 50px" src="../template/img/administrador.png">
                                <?php } else { ?>
                                    <img class="img-profile rounded-circle" style="width: 50px" src="../template/img/vendedor.png">
                                <?php } ?>
                            </td>
                            ...
                            <td>
                                <button class="btn btn-outline-warning" data-toggle="modal" data-target="#modalModificarUsuario" onclick="obtenerDatosActualizarUsuario('<?php echo $datos ?>')">
                            ...
                            <td>
                                <button class="btn btn-outline-danger" onclick="eliminarUsuario('<?php echo $row['USU_ID'] ?>')">

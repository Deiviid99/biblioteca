<?php require_once("../controller/autorController.php"); ?>
<div class="card shadow mb-4">
    <div class="card-header d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold ">Registro de Autores</h6>
        <button class="btn btn-outline-info" data-toggle="modal" data-target="#modalAgregarAutor">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" style="text-align: center;" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody style="color: #545454">
                    <?php
                    $dato = new Autor();
                    $autor = $dato->obtenerAutor();
                    $cont = 0;
                    while ($row = mysqli_fetch_array($autor)) {
                        $cont = $cont + 1;
                        $datos = $row["AUT_ID"] . "||" . $row["AUT_NOMBRE"] . "||" . $row["AUT_APELLIDO"];
                    ?>
                        <tr>
                            <td><?php echo $cont; ?></td>
                            <td><?php echo $row['AUT_NOMBRE'] . " " . $row["AUT_APELLIDO"]; ?></td>
                            <td>
                                <button class="btn btn-outline-warning" data-toggle="modal" data-target="#modalModificarAutor" onclick="obtenerDatosActualizarAutor('<?php echo $datos ?>')">
                            ...
                            <td>
                                <button class="btn btn-outline-danger" onclick="eliminarAutor('<?php echo $row['AUT_ID'] ?>')">
